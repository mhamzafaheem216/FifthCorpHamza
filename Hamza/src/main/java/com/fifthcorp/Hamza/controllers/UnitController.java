...
import com.fifthcorp.Hamza.services.UnitService;
...
public class UnitController {
    ...
    @GetMapping("fetchAll")
    @ApiOperation(value = "Fetch All Units")
    ...
    @ApiOperation(value = "Get Units By Owner")
    ...
    public ResponseEntity<Response> create(@RequestBody String obj, HttpServletRequest request) {
        ...
        try {
            Unit createdUnit = unitService.createUnit(obj);
    ...
    @PostMapping("/update")
    @ApiOperation(value = "Update existing Unit")
    public ResponseEntity<Object> update(@RequestBody String obj, HttpServletRequest request) {
        if (!Helper.validateToken(request)) {
            return new ResponseEntity<>(new Response("Unauthorized: Invalid token"), HttpStatus.UNAUTHORIZED);
        }

        try {
            Unit updatedUnit = unitService.updateUnit(obj);
    ...
    @ApiOperation(value = "Change Unit Owner")
    public ResponseEntity<Object> changeUnitOwner(@RequestBody String obj, HttpServletRequest request) {
        if (!Helper.validateToken(request)) {
            return new ResponseEntity<>(new Response("Unauthorized: Invalid token"), HttpStatus.UNAUTHORIZED);
        }

        try {
            Unit updatedUnit = unitService.changeUnitOwner(obj);
            return new ResponseEntity<>(updatedUnit, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(new Response(e.getMessage()), HttpStatus.INTERNAL_SERVER_ERROR);
        }
